...
from config.res18_cifar100_whole import Config
from utils.dataset import InfoCIFAR100
from utils.ma_policy import MaPruningPolicy
...
if __name__ == "__main__":
    ...
    train_dataset = InfoCIFAR100(root='/sources/dataset/cifar100', train=True, download=True, transform=train_tranform)
    test_datset = datasets.CIFAR100(root='/sources/dataset/cifar100', train=False, download=True, transform=test_transform)

    # Policy
    policy = MaPruningPolicy(len(train_dataset), config.num_epochs, prob=0.7)
    ...
    # mlflow setting
    mlflow.set_experiment('R18_CIFAR100_IB_MA')
    mlflow.start_run()
