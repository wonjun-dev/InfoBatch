...
class ReversePruningPolicy(Sampler):
    ...
    def _update_pruned_idxs(self):
        ...
        self.use_idxs = torch.cat((d2, pruned), dim=0)

        d2_scaler = torch.ones(len(d2))
        # d3_scaler = torch.ones(len(d3)) * (1./(1.-self.prob + 1e-6))
        pruned_scaler = torch.ones(len(pruned)) * (1. / self.prob)
        self.scaler = torch.cat((d2_scaler, pruned_scaler), dim=0)

        self.pruned[pruned] = False
        self.pruned[d3] = True
...
if __name__ == "__main__":
    ...
    policy = PruningPolicy(len(dataset))
